...
**Verified fact (2026-09-04):** `tfroot-github/main.tf` is the canonical organization repository-policy inventory. At revision `aca473b319f52fd5a562effe71b4516cdbd89009`, `agent-knowledge` and `channel-project` are private and the three historical Ansible repositories are archived.
...
5. **Retire static writers:** remove the per-repository public `git-sync` containers only after the cutover and lifecycle checks succeed. The credentialed private-allowlist containers in `repo-cache-sync-private.yaml` are outside this design and are not retired by it.
