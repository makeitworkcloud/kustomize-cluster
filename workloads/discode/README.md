...
- **Archive integrity:** the init script enforces both the immutable commit
  ...
  the check fails closed and the digest must be recomputed. The `discode-build`
  CI job independently passed this same check on a GitHub runner
  (run 35464439305).
...
Observed: run 35464439305 (PR #249, head `8468285c`) completed with all
three jobs green — the runner pulled the pinned digest, passed the archive
sha256 check (`/tmp/src.tar.gz: OK`), installed 88 packages via `npm ci`,
built with `tsc`, and pruned to 38 production packages with no vitest
binary.
